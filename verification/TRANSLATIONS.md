# GPS Baibbat — the three languages, string by string
...
| **!** | `position_stale` | This is where you were, not where you are | C’est là où vous étiez, pas là où vous êtes | هذا مكانك السابق لا مكانك الحالي |
...
## Location availability — three distinct problems, three remedies

| | key | English | Français | العربية |
|---|---|---|---|---|
| **!** | `location_off_title` | Location is switched off | La localisation est désactivée | خدمة الموقع معطّلة |
| **!** | `location_off_body` | You gave this app permission, but the device’s location switch is off, so no app can get a position. The arrow cannot work until you turn it on. | Vous avez autorisé cette application, mais la localisation de l’appareil est désactivée : aucune application ne peut obtenir de position. La flèche ne fonctionnera pas tant qu’elle n’est pas activée. | لقد منحت الإذن لهذا التطبيق، لكن مفتاح الموقع في الجهاز مطفأ، فلا يمكن لأي تطبيق أن يحدد الموقع. لن يعمل السهم حتى تفعّله. |
|  | `location_off_button` | Turn on location | Activer la localisation | تفعيل الموقع |
| **!** | `acquiring_title` | Searching for satellites | Recherche de satellites | جارٍ البحث عن الأقمار الصناعية |
| **!** | `acquiring_body` | %1$s of %2$s satellites in view. With no internet this takes 30 to 90 seconds from cold, sometimes longer. Go outside or stand where you can see sky. | %1$s satellites sur %2$s en vue. Sans Internet, cela prend de 30 à 90 secondes à froid, parfois davantage. Sortez ou placez-vous là où vous voyez le ciel. | %1$s من %2$s قمرًا في المدى. بدون إنترنت يستغرق ذلك من 30 إلى 90 ثانية عند البدء، وربما أكثر. اخرج إلى العراء أو قف حيث ترى السماء. |
...
|  | `map_none_explanation` | Areas are downloaded once while you’re online, then work offline forever after. | Les zones se téléchargent une fois en ligne, puis fonctionnent hors ligne pour toujours. | تُنزَّل المناطق مرة واحدة أثناء الاتصال، ثم تعمل بدون اتصال إلى الأبد. |
|  | `map_open_region_list` | Open the areas list | Ouvrir la liste des zones | فتح قائمة المناطق |
...
|  | `area_also_covers` | Also covers your position | Couvre aussi votre position | تغطي موقعك أيضاً |
|  | `area_recommended` | Recommended for your position | Recommandée pour votre position | موصى بها لموقعك |
| **!** | `area_position_unknown` | No position fix yet, so coverage can’t be checked | Position non acquise : impossible de vérifier la couverture | لم يُحدَّد الموقع بعد، لذا يتعذّر التحقق من التغطية |
|  | `level_standard` | Standard | Standard | قياسي |
|  | `level_detailed` | Detailed | Détaillé | مفصّل |
|  | `level_standard_summary` | Roads, tracks, towns and villages | Routes, pistes, villes et villages | الطرق والمسالك والمدن والقرى |
|  | `level_detailed_summary` | Adds footpaths, isolated buildings and seasonal watercourses | Ajoute les sentiers, les bâtiments isolés et les cours d’eau saisonniers | يضيف المسارات والمباني المعزولة والأودية الموسمية |
|  | `areas_one_level_note` | One detail level per area. Choosing another replaces the one you have. | Un seul niveau de détail par zone. En choisir un autre remplace celui que vous avez. | مستوى تفصيل واحد لكل منطقة. اختيار مستوى آخر يستبدل المستوى الحالي. |
|  | `areas_installed` | Installed | Installée | مثبَّتة |
|  | `areas_storage_used` | %1$s used by maps | %1$s utilisés par les cartes | %1$s تستخدمها الخرائط |
|  | `areas_free_space` | %1$s free on this device | %1$s libres sur cet appareil | %1$s متاحة على هذا الجهاز |
|  | `areas_file_ok` | File verified · %1$s on disk | Fichier vérifié · %1$s sur le disque | تم التحقق من الملف · %1$s على القرص |
| **!** | `areas_file_bad` | This file failed its check (%1$s). Delete and download it again. | Ce fichier a échoué à la vérification (%1$s). Supprimez-le et retéléchargez-le. | فشل هذا الملف في التحقق (%1$s). احذفه ونزّله مرة أخرى. |
...
|  | `metered_title` | You’re on mobile data | Vous êtes en données mobiles | أنت تستخدم بيانات الجوال |
|  | `metered_body` | This download will use %1$s of mobile data. | Ce téléchargement utilisera %1$s de données mobiles. | سيستهلك هذا التنزيل %1$s من بيانات الجوال. |
...
|  | `download_verifying` | Checking the file… | Vérification du fichier… | جارٍ التحقق من الملف… |
...
|  | `download_no_space` | Not enough space — needs %1$s, %2$s free | Espace insuffisant — il faut %1$s, %2$s libres | المساحة غير كافية — يلزم %1$s، والمتاح %2$s |
|  | `download_network_failed` | Connection lost. Your progress is kept — resume when you have signal. | Connexion perdue. Votre progression est conservée — reprenez dès que vous aurez du réseau. | انقطع الاتصال. تم حفظ تقدّمك — استأنف عند توفر الشبكة. |
| **!** | `download_corrupt` | The file arrived damaged and was deleted. Nothing was installed. | Le fichier est arrivé endommagé et a été supprimé. Rien n’a été installé. | وصل الملف تالفًا وتم حذفه. لم يُثبَّت أي شيء. |
|  | `download_server_error` | The server refused the download (%1$s). | Le serveur a refusé le téléchargement (%1$s). | رفض الخادم التنزيل (%1$s). |
|  | `download_replacing` | Your current level stays until this finishes | Votre niveau actuel reste en place jusqu’à la fin | يبقى مستواك الحالي حتى ينتهي هذا |
...
|  | `diag_course_chip` | course from receiver | cap du récepteur | الاتجاه من المستقبِل |
|  | `diag_course_derived` | course from movement | cap calculé par le déplacement | الاتجاه من الحركة |
|  | `diag_course_trust` | receiver course trust | confiance dans le cap du récepteur | الثقة باتجاه المستقبِل |
|  | `diag_course_trusted` | trusted | fiable | موثوق |
| **!** | `diag_course_distrusted` | stale — using movement | obsolète — cap calculé utilisé | قديم — يُستخدم الاتجاه من الحركة |
